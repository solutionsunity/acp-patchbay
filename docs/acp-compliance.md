...
| Preemptively mark incomplete tool calls cancelled | ✅ | Fixed 2026-07-11 (G6), revised 2026-07-11 — a *sidecar fact* (`ToolCallBlock.interrupted`), not the ACP `status` field: the wire has no cancelled tool status, so patchbay doesn't put words in it. Originally derived per-render from "is any turn active in this session right now" — found to bleed: a cancelled turn's stuck call would revive its spinner the instant a *later* turn started in the same session, since that flag is session-scoped, not turn-scoped. Now set once, at the real quiet points, by `session-manager.ts`'s sweep (`sweepOpenToolCalls`, the tool-call analogue of `broker.cancelPending` for permission requests — G2's own pattern, generalized): **every** turn end (any stop reason — even a claimed clean `end_turn` leaves nothing running, so a call still open then is stranded; the error path included) and **replay end** (`loadSilently` — live cancel and its later session/load replay render identically), each sweeping the session's still-open ids (`isToolCallOpen`, the one spelling of "not yet terminal") as `toolCallInterrupted` before the closing event lands. A trailing `tool_call_update` still wins — any fresh upsert clears the flag, since the wire is still talking about the call after all. A persisted last-known view (`transcriptSeeded`, resume-without-replay) normalizes the same way: a snapshot with no live turn behind it is interrupted on sight. |
...
| Receive: non-text in `agent_message_chunk` (image/audio/resource_link/resource) | 🟡 | The *silent drop* is fixed (2026-07-11, G4): a closed in-place placeholder block, type-labeled (`*[image content — not rendered]*`). `resource_link` now renders for real (2026-07-12, G10b): `@name` merged into the user prose run / `[name](uri)` markdown link in agent prose. Still floored: embedded **text-formed resource** (renderable text — the closest thing to a violation of this section's preamble in the codebase), image, audio/blob (→ G10). |
...
| G11 | Honesty | Thought-chunk path missed G4's sweep: non-text thought content was still a *silent* drop, and whitespace-only chunks opened blank "Thought" accordions (observed on claude-agent-acp `session/load` replay) | **Fixed 2026-07-12** — non-text thought content gets the same type-labeled placeholder as the message chunks; a whitespace-only chunk never *opens* a prose/thought/user run (an open run still takes it — mid-stream spacing is real content) and never severs a neighboring run. |
| G10 | SHOULD | Non-text message content renders as placeholder only (§8) | **(b) resolved 2026-07-12**: `resource_link` renders — in a user chunk as the `@name` mention the user typed, merged INTO the prose run (a placeholder block was severing one replayed prompt into bubble + placeholder + bubble — our own positional mentions ride this type); in an agent chunk as a `[name](uri)` markdown link. Remaining open, each its own design discussion: **(a)** embedded text-formed `resource` — renderable text today, needs only a labeled text render; **(c)** image — needs an actual rendering + CSP decision (data: images are already allowed for diagrams; an `<img>` block is a deliberate, recorded widening if taken); **(d)** audio / blob-formed resource — placeholder genuinely is the floor until a playback/save surface is justified. Not a patch — sequence alongside G5's phase-B rendering work. |
