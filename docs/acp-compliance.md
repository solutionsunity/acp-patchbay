...
| Content `diff` | ✅ | Rendered where it was reported: the tool card counts the lines each call's diffs add and remove and opens them in the native diff editor (`stashToolDiffs`, `toolCallDiff`). The spec's "the original content" doesn't say whole file, and agents differ — whole files (Gemini, Codex) or changed regions (Claude, OpenCode's edit input) — so each entry is counted only against its own counterpart; several regions of one path sum and open joined. A missing `oldText` counts every new line as added, as the agent said. No session-wide total: a file the agent writes itself leaves the client no trustworthy "before". |
