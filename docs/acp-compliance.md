...
| Receive: `ContentChunk.messageId` (message identity on every chunk) | ✅ | Governs the user-run merge: same id continues the run, a new id — or an id-less chunk — is its own message (id-less agents replay whole messages per chunk; an agent splitting one message across id-less chunks would be unreconstructable by any client). Wire-verified 2026-07-12: claude-agent-acp 0.58.1 replays multi-part prompts as several chunks under ONE id and adjacent messages under distinct ids; auggie 0.32.0 omits the field. Agent/thought runs stay on the block-interruption rule — live streaming deltas, correct there. |
